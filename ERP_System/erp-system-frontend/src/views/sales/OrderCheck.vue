    ...
    <el-dialog v-model="dialogVisible" title="订单详情" class="custom-dialog" width="80%" height="90%">
            ...
            <template #default="{row}">
              <el-table :data="[row]" border>
                ...
                <el-table-column label="商品名称">
                  <template #default="{row}">
                    <span v-if="row.productName !== '无'">{{ row.productName }}</span><span v-else>无</span>
                  </template>
                </el-table-column>
      ...
      <template #footer>
        <div class="footer-buttons">
          <el-button v-if="employee.position === '部长' && salesOrder.items[0].status === '等待部长审批'" type="success" @click="acceptOrder" class="decision-button">通过</el-button>
          <el-button v-if="employee.position === '部长' && salesOrder.items[0].status === '等待部长审批'" type="danger" @click="rejectOrder" class="decision-button">打回</el-button>
          <el-button v-if="employee.position === '部员' && salesOrder.items[0].status === '部长打回'" type="success" @click="submitOrderAgain" class="decision-button">再次提交</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
const loadData = (page = 1) => {
  ...
  axios.post('/api/sales/searchSalesOrder', searchCriteria)
      .then(response => {
...
const searchOrderDetail = (row) => {
  ...
  axios.post('/api/sales/searchSalesOrderDetail', { id: selectedId.value })
      .then(response => {
...
const acceptOrder = ()=>{
  ...
  axios.post('/api/sales/acceptSalesOrder', requestData)
      .then(res=>{
...
const rejectOrder = ()=>{
  ...
  axios.post('/api/sales/rejectSalesOrder', requestData)
      .then(res=>{
        if(res.data.code==='200'){
...
const submitOrderAgain = ()=>{
  ...
  axios.post('/api/sales/submitSalesOrderAgain', requestData)
      .then(res=>{
        if(res.data.code==='200'){
