    ...
    <el-card class="home-card">
      <div class="home-content">
        <el-row>
          <el-col :span="24">
            <h1>欢迎使用轧钢厂管理系统！</h1>
            <p class="department-info">您所在的部门是：采购部！</p>
...
onMounted(async () => {
  try {
    const salesResponse = await axios.get('/api/purchasing/totalPurchase');
    totalSales.value = salesResponse.data.total;

    const monthlySalesResponse = await axios.get('/api/purchasing/monthlyPurchasing');
    monthlySales.value = monthlySalesResponse.data.monthly;

    const todayOrdersResponse = await axios.get('/api/purchasing/todayOrders');
...
onMounted(async () => {
  try {
    const response = await axios.get('/api/purchasing/purchasingTrend');
...
onMounted(async () => {
  try {
    const response = await axios.get('/api/purchasing/ordersDistribution');
